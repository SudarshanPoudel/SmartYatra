...
export const MultiOptionSelector = ({ label_text, given_options, onOptionsChange }) => {
  ...
  const handleActivities = (event, option) => {
    ...
    setOptions(prevOptions => prevOptions.includes(option) ? prevOptions.filter(p => p !== option) : [...prevOptions, option]
    );
  };
  ...
  const handleInputChange = (event) => {
    ...
    if (value) {
      const filtered = given_options.filter(option =>
        option.name.toLowerCase().includes(value.toLowerCase()) && !options.includes(option.name)).slice(0,5);
    ...
    <div className="options-container">
      ...
      <div className="btns">
        {given_options.slice(0, 6).map((e, i) => (
          <button className={options.includes(e.name) ? "option-btn-selected option-btn" : "option-btn"} type="button" key={i} onClick={(event) => handleActivities(event, e.name)}>
            <img src={getImageURL(e.image)} alt="#" className="buttonBG" />
        ...
        <div className="selected-options">
          {options.map((option, index) => (
            <div key={index} className="tag">
              ...
              <button onClick={(event) => handleTagRemove(event, option)}style={{ marginLeft: '5px', color: 'red' }}> &times;</button>
            </div>
          ))}
        </div>
      )}
    ...
    <div className="options-container">
      ...
      <div className="btns">
        {given_options.map((option, index) => (
          <button className={selectedOption === option.name ? "option-btn-selected option-btn": "option-btn"} type="button" key={index} onClick={(event) => handleSelectOption(event, option.name)}>
